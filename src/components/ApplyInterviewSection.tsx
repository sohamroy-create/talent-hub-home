  ...
  return (
    <div className="flex items-center justify-center gap-3 mt-3">
      ...
      <div className="flex gap-2">
        {units.map((u) => (
          <div key={u.label} className="flex flex-col items-center bg-background/60 border border-border/50 rounded-md px-4 py-2 min-w-[56px]">
            <span className="text-lg font-extrabold text-foreground leading-none tabular-nums">{String(u.value).padStart(2, "0")}</span>
            <span className="text-[11px] font-bold uppercase tracking-widest text-muted-foreground">{u.label}</span>
          </div>
        ))}
      </div>
    </div>
